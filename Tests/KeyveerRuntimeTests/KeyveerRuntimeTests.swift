...
final class KeyveerRuntimeTests: XCTestCase {
  ...
  func testDefaultConfigurationUsesSchemaV3ActivationWithoutEscapeBinding() throws {
    ...
    XCTAssertEqual(trail["bendOffsetDistanceMax"] as? Double, 50)
    XCTAssertEqual(trail["bendOffsetDirectionMin"] as? Double, -90)
    XCTAssertEqual(trail["bendOffsetDirectionMax"] as? Double, 90)
    XCTAssertEqual(trail["bendSpacingMin"] as? Double, 30)
    XCTAssertEqual(trail["bendSpacingMax"] as? Double, 100)
    XCTAssertEqual(trail["trunkWidthScaleMin"] as? Double, 0.1)
    XCTAssertEqual(trail["trunkWidthScaleMax"] as? Double, 2)
    XCTAssertEqual(trail["arcWidthScaleMin"] as? Double, 0.25)
    XCTAssertEqual(trail["arcWidthScaleMax"] as? Double, 0.8)
    XCTAssertEqual(trail["arcLengthMin"] as? Double, 300)
    XCTAssertEqual(trail["arcLengthMax"] as? Double, 500)
    XCTAssertEqual(trail["arcGapMin"] as? Double, 200)
    XCTAssertEqual(trail["arcGapMax"] as? Double, 300)
    XCTAssertEqual(trail["arcHoldMin"] as? Double, 0.20)
    XCTAssertEqual(trail["arcHoldMax"] as? Double, 1)
  ...
  func testVisualConfigurationDefaultsAndPartialValuesAreAccepted() throws {
    let defaults = try JSONDecoder()
      ...
      .validated().visual
    XCTAssertEqual(defaults.marker.coreDiameter, 12)
    XCTAssertEqual(defaults.marker.glowRadius, 3)
    XCTAssertEqual(defaults.marker.outerGlowColor, "#F1DD20")
    XCTAssertEqual(defaults.marker.outerGlowOpacity, 1)
    XCTAssertEqual(defaults.marker.glowStrength, 1.0)
    XCTAssertEqual(defaults.trail.coreWidth, 5)
    XCTAssertEqual(defaults.trail.trunkWidthScaleMin, 0.1)
    XCTAssertEqual(defaults.trail.trunkWidthScaleMax, 2)
    XCTAssertEqual(defaults.trail.arcWidthScaleMin, 0.25)
    XCTAssertEqual(defaults.trail.arcWidthScaleMax, 0.8)
    XCTAssertEqual(defaults.trail.bendSpacingMin, 30)
    XCTAssertEqual(defaults.trail.bendSpacingMax, 100)
    XCTAssertEqual(defaults.trail.bendOffsetDistanceMin, 6)
    XCTAssertEqual(defaults.trail.bendOffsetDistanceMax, 50)
    XCTAssertEqual(defaults.trail.arcLengthMin, 300)
    XCTAssertEqual(defaults.trail.arcLengthMax, 500)
    XCTAssertEqual(defaults.trail.arcGapMin, 200)
    XCTAssertEqual(defaults.trail.arcGapMax, 300)
    XCTAssertEqual(defaults.trail.arcHoldMin, 0.2)
    XCTAssertEqual(defaults.trail.arcHoldMax, 1)
    XCTAssertEqual(defaults.trail.blurRadius, 6)
    XCTAssertEqual(defaults.trail.outerGlowColor, "#F1DD20")
    XCTAssertEqual(defaults.trail.outerGlowOpacity, 1)
    XCTAssertEqual(defaults.trail.glowStrength, 3)
    ...
    XCTAssertEqual(partialDecoded.visual.trail.bendSpacingMin, 30)
    XCTAssertEqual(partialDecoded.visual.trail.bendSpacingMax, 100)
    XCTAssertEqual(partialDecoded.visual.trail.arcLengthMin, 300)
    XCTAssertEqual(partialDecoded.visual.trail.arcLengthMax, 500)
    XCTAssertEqual(partialDecoded.visual.trail.arcGapMin, 200)
    XCTAssertEqual(partialDecoded.visual.trail.arcGapMax, 300)
    XCTAssertEqual(partialDecoded.visual.trail.arcHoldMin, 0.20)
    XCTAssertEqual(partialDecoded.visual.trail.arcHoldMax, 1)
