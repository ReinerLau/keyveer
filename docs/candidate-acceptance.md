...
- [ ] One continuous pointer movement grows one white-core, pure-blue-glow lightning trunk from its exact origin to the marker after the first 8pt; the whole trunk remains visible without a length cap, full-width branches, or sparks, with one continuous thin companion arc covering the same origin-to-marker range.
