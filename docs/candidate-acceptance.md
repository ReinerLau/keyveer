...
- [ ] One continuous pointer movement grows one white-core, pure-blue-glow lightning trunk from its exact origin to the marker after the first 8pt; the whole trunk remains visible without a length cap, full-width branches, or sparks, while the companion arc appears as deterministic 80–180pt sections separated by 24–72pt path gaps.
- [ ] During keyboard movement, the default and precision-slow speeds hide the prominent trunk while retaining the subtle companion arc; holding any configured fast-speed key shows the trunk again. Physical mouse movement remains eligible for the trail.
