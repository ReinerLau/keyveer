...
- [ ] One continuous pointer movement grows one white-core, pure-blue-glow lightning trunk from its exact origin to the marker after the first 8pt; the whole trunk remains visible without a length cap, full-width branches, or sparks, while independently generated thinner companion polylines follow the trunk direction, begin at random path distances, grow to independent random lengths, remain for independent random hold times after growth stops, and disappear individually without a simultaneous-count limit.
- [ ] During continuous trail growth, the trunk and each companion arc independently hide for exactly one or two display frames at low random frequency; stopped dissipation and Reduce Motion do not add this flicker.
- [ ] During keyboard movement, the default and precision-slow speeds hide the prominent trunk while retaining independently flickering companion arcs; holding any configured fast-speed key shows the trunk again. Physical mouse movement remains eligible for the trail.
- [ ] Straight, curved, reversing, and closed-loop movement extend the same trunk along the complete motion route; major bends are visibly sparse (about 30pt apart) with no extra inter-bend detail points, bend positions, offsets, and 0.45–1.6× core widths are irregular, frozen once committed, and use the same position-independent two-dimensional offset distribution along the trunk.
- [ ] Releasing the final keyboard movement key immediately stops growth; for physical pointer-only movement, 100ms without position change is the fallback. The opaque trunk then freezes in global coordinates and sections across its whole length shrink and disconnect in place for 0.45s, with no directional wipe or flying fragments.
